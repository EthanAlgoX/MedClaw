...
class AgentLoop:
    ...
    def __init__(
        ...
        self.use_cases = MedicalResearchUseCases(workspace)
    ...
    async def run(self, user_message: str, history: list[dict[str, Any]] | None = None) -> str:
        ...
        workflow_response = await self.use_cases.run_query(user_message, self.provider)
        if workflow_response:
            logger.info("Answered query through research workflow")
            self.memory_store.save(
                user_message,
                {"response": workflow_response, "path": "research_workflow"},
            )
